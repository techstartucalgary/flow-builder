...
const ROOM_EXTRACTION_VERSION = '2026-04-room-review-v2';
...
function shouldExtractRooms(doc: AnnotationDocument): boolean {
  ...
  if (typeof doc.baseImage.scalePxPerFt === 'number' && doc.baseImage.scalePxPerFt > 0) {
    const allRoomsZeroArea = rooms.every((room) => {
      const area = (room.relations as { areaSqFt?: number } | undefined)?.areaSqFt;
      return typeof area !== 'number' || area <= 0;
    });
    if (allRoomsZeroArea) return true;
  }
  return false;
}
...
export default function AnnotationEditorShell({
  ...
  const refreshRoomsFromDocument = useCallback(async (
    ...
    if (extracted.status === 'no_scale') {
      setWarning('Set the drawing scale before running room detection.');
      return {
        document: baseDoc,
        preservedExistingRooms: true,
        replacedRooms: false,
        summary: {
          roomCount: 0,
          spaceCount: 0,
    ...
    const lowQualityExtraction = extracted.rooms.length === 0;
    const skipPersist = options?.persist === false
      || (lowQualityExtraction && existingRooms.length === 0);
    if (skipPersist) {
      return {
        document: nextDoc,
  ...
  const loadDocument = useCallback(async () => {
    ...
    try {
      ...
      if (currentStoreDocument?.projectId === projectId && currentStoreDocument.page === pageNumber) {
        ...
        if (!hasManualGeometryEdits(currentStoreDocument)) {
          ...
          if (!completedStartupRoomRefreshes.has(roomRefreshKey)) {
            ...
            void refreshRoomsFromDocument(currentStoreDocument, currentStoreDocument.meta.revision)
              .then(() => {
                setStatusMessage(null);
              })
              .catch((err: any) => {
      ...
      if (existing.document) {
        ...
        if (!hasManualGeometryEdits(existingDoc)) {
          ...
          if (!completedStartupRoomRefreshes.has(roomRefreshKey)) {
            completedStartupRoomRefreshes.add(roomRefreshKey);
            setStatusMessage('Refreshing room extraction...');
            void refreshRoomsFromDocument(existingDoc, existing.latest_revision)
              .then(() => {
                setStatusMessage(null);
              })
              .catch((err: any) => {
      ...
      if (!completedStartupRoomRefreshes.has(roomRefreshKey)) {
        completedStartupRoomRefreshes.add(roomRefreshKey);
        setStatusMessage('Refreshing room extraction...');
        void refreshRoomsFromDocument(persistedInitialDoc, savedInitial.latest_revision, { persist: false })
          .then(() => {
            setStatusMessage(null);
          })
          .catch((err: any) => {
